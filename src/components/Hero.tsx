...
const Hero = () => {
  ...
  const mascotY = useTransform(scrollY, [0, 300], enableHeavyMotion ? [0, 30] : [0, 0]);
  ...
  return <>
      <section className="relative min-h-screen md:min-h-[85vh] flex items-center pt-32 pb-16 md:pt-40 md:pb-20 overflow-hidden" id="home" aria-labelledby="hero-heading">
        ...
        <div className="container mx-auto px-4 md:px-6 z-10 relative">
          ...
          delay: 0.2
        }} className="text-center text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold mb-8 md:mb-12 tracking-tight" style={{
          textShadow: "0 4px 30px rgba(0, 0, 0, 0.5), 0 2px 10px rgba(0, 0, 0, 0.3)"
        }}>
            <span className="text-[#ec9a36]">Jim's</span>{" "}
            <span className="text-[#ec9d36]">Dumpster Services</span>
          </motion.h2>

          {/* Two Column Layout - Mascot Left, Content Right */}
          <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 lg:gap-16">
            {/* Mascot Section - Left Side with Glow */}
            <motion.div className="relative flex items-center justify-center pointer-events-none md:w-[40%]" style={{
            y: isMobile ? 0 : mascotY
          }} variants={mascotVariants} initial="hidden" animate="visible">
              ...
              {/* Mascot image */}
              <motion.img src={jdMascot} alt="Jim's Dumpster Services Mascot" className="relative z-10 h-[35vh] md:h-[45vh] lg:h-[55vh] w-auto object-contain" style={{
              filter: "drop-shadow(0 0 60px rgba(16, 185, 129, 0.4)) drop-shadow(0 0 100px rgba(16, 185, 129, 0.2))"
            }} />
            </motion.div>

            {/* Content Section - Right Side */}
            <motion.div className="max-w-xl text-center md:text-left md:w-[50%]" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
            once: true,
            margin: "-100px"
          }}>
            {/* Exclusive badge */}
            <motion.div variants={itemVariants} className={`inline-flex items-center gap-2 px-4 py-2 mb-4 md:mb-5 rounded-full bg-white/10 border border-white/20 text-white/90 text-xs md:text-sm font-medium ${isMobile ? '' : 'backdrop-blur-md'}`}>
              <Shield className="h-3.5 w-3.5 md:h-4 md:w-4 text-emerald-400" />
              <span>Family-owned • Driveway Safe • Same-Day Available</span>
            </motion.div>

            {/* Main Headline */}
            <motion.h1 id="hero-heading" variants={itemVariants} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-white mb-4 md:mb-6 leading-tight">
              Driveway Safe{" "}
              <span className="block">Dumpster Rentals</span>
            </motion.h1>

            {/* Supporting Paragraph */}
            <motion.p variants={itemVariants} className="text-base sm:text-lg md:text-xl text-white/90 mb-6 md:mb-8 leading-relaxed max-w-xl md:max-w-lg">
              Professional dumpster rental service that respects your time, your
              property, and your peace of mind.
            </motion.p>

            {/* CTA Buttons */}
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-6 md:mb-8 justify-center md:justify-start">
              <Link to="/quote" className="w-full sm:w-auto">
                <motion.div whileHover={shouldReduceMotion ? {} : {
                  ...
                  duration: MOTION_CONFIG.duration.button
                }}>
              ...
              <a href={PHONE_HREF} className="w-full sm:w-auto">
                <motion.div whileHover={shouldReduceMotion ? {} : {
                  ...
                  <Button size="lg" className={`w-full sm:w-auto bg-white/10 hover:bg-white/20 text-white font-semibold rounded-full px-6 md:px-8 py-5 md:py-6 text-sm md:text-base border-2 border-white/40 hover:border-white/60 transition-all duration-200 shadow-lg group ${isMobile ? '' : 'backdrop-blur-md'}`}>
                    <motion.div className="inline-flex items-center" whileHover={shouldReduceMotion ? {} : {
                      x: 2
                    }} transition={{
                      duration: 0.15
                    }}>
            ...
            {/* Trust Indicators */}
            <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 sm:gap-6 text-white/80 text-xs md:text-sm justify-center md:justify-start">
      ...
      ease: "easeOut"
    }} className="fixed bottom-0 left-0 right-0 z-40 md:hidden pb-safe pointer-events-none" style={{
        ...
        <div className={`bg-background/95 ${isMobile ? '' : 'backdrop-blur-md'} border-t border-border shadow-2xl px-4 py-3 pointer-events-auto`}>
          <div className="flex gap-3 max-w-md mx-auto">
            <Link to="/quote" className="flex-1">
              <Button size="sm" className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-semibold rounded-full px-6 py-5 text-sm shadow-lg shadow-emerald-500/30">
                Book Now
              </Button>
            </Link>
